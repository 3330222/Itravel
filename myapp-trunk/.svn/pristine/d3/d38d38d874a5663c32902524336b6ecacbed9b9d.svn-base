(function($, undefined) {
	...
	$.CircleEventManager.prototype = {
		_init : function(options) {
			...
			this.options = $.extend(true, {}, $.CircleEventManager.defaults,
					options);
			...
			this.$el
					.on({
						'mouseenter.circlemouse' : function(event) {
							...
							circleWidth = el.outerWidth(true), circleHeight = el
									.outerHeight(true), circleLeft = el
									.offset().left, circleTop = el.offset().top, circlePos = {
							...
							if (_self.$el.css('cursor') === 'pointer'
									|| _self.$el.is('a'))
								cursor = 'pointer';
							...
							el.on('mousemove.circlemouse', function(event) {
								...
								var distance = Math.sqrt(Math.pow(event.pageX
										- circlePos.x, 2)
										+ Math
												.pow(event.pageY - circlePos.y,
														2));

								if (!Modernizr.borderradius) {

									// inside element / circle
									el.css('cursor', el.data('cursor')).data(
											'inside', true);
	...
	$.fn.circlemouse = function(options) {
		...
		if (typeof options === 'string') {
			...
			this.each(function() {
				...
				var instance = $.data(this, 'circlemouse');
				if (!instance) {
					$.data(this, 'circlemouse', new $.CircleEventManager(
							options, this));
				}
